...
public class Settings {
    ...
    public void load(DDPlugin plugin) {
        ...
        m_apparel_force_wear = this.m_plugin.Config.Bind<string>("General", "Force Wear", "", "Comma-separated list of apparel items (i.e. Crop_Top_01, Polo_Shirt_01, etc) exactly matching (no wildcards) the second subfolder (directly under Female/Male) of the mod/dump textures folder.  The mod will make its best effort to force all customers (of the apparel's gender) to wear the specified item(s).  If two or more apparel items that apply to the same gender and clothing slot (i.e. Polo_Shirt_01 and Polo_Shirt_02) are specified then the mod will randomly select between them.  Check LogOutput.txt for information if this setting does not seem to work and see the Nexus homepage for this mod for troubleshooting tips.  Changes to this setting require a game reload.");
